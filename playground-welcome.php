...
class Playground_Welcome {
    ...
    public static function get_vision_page_path() {
        $locale = determine_locale();
        $lang = substr($locale, 0, 2);
        $plugin_dir = plugin_dir_path(__FILE__);

        $localized_file = $plugin_dir . $lang . '/vision-page.html';
        if (file_exists($localized_file)) {
            return $localized_file;
        }

        return $plugin_dir . 'vision-page.html';
    }

    public static function get_vision_page_content() {
        $path = self::get_vision_page_path();
        if (file_exists($path)) {
            return file_get_contents($path);
        }
        return '';
    ...
    public static function activate() {
        ...
        if ($sample_page) {
            wp_update_post([
                'ID' => $sample_page->ID,
                'post_title' => __('The Vision', 'playground-welcome'),
                'post_content' => self::get_vision_page_content(),
                'post_name' => 'the-vision',
            ]);
        }
    }
